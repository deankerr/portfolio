...
That opacity is the boundary. Everything IRC-specific — reconnection, buffer routing, self-echo — lives in the plugin, one layer above the iirc-lib protocol library. The SolidJS web client renders it all in a multi-buffer layout, deriving view state rather than syncing it.
